...
test('release version strings are derived from package metadata instead of duplicated literals', () => {
  ...
  assert.match(metadataSource, /export const appReleaseTag = `v\$\{appVersion\}-windows-x64-latest`/)
  ...
  assert.match(workflowSource, /\$releaseTag = "v\$version-windows-x64-latest"/)
  assert.match(workflowSource, /\$productName = \$package\.build\.productName/)
  assert.match(workflowSource, /\$files = @\(/)
