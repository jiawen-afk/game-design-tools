...
test('project management exposes remote database and qiniu kodo configuration', () => {
  ...
  assert.match(updateRemoteProjectLinksSource, /remoteDeviceBindingResolver\.bindProjectToCurrentDevice\(\s*projectId,\s*settingsWorkspace\.selectedDatabaseProfileId,\s*settingsWorkspace\.selectedKodoProfileId/s)
  assert.match(updateRemoteProjectLinksSource, /const updated = await remoteProjectRepository\.updateProject\(projectId, input\)/)
...
test('project workspace startup resolves migrated projects from the remote repository first', () => {
  ...
  assert.match(workspaceSource, /无法读取远程项目列表/)
...
test('matte task groups can export group images and collect them to public image assets', () => {
  ...
  assert.match(hookSource, /writeCurrentProjectSpaceState/)
