...
test('personal space resource kinds are first-level tabs instead of a common resource tab', () => {
  ...
  assert.match(sectionsSource, /onTransferGroup/)
  ...
  assert.match(resourceSectionBody, /selectedGroup=\{selectedGroup\}/)
  assert.match(resourceSectionBody, /onSelectedGroupChange=\{setSelectedGroup\}/)
  assert.match(resourceSectionBody, /const groupAssets = section\.assets\.filter\(\(item\) => item\.groupName === selectedGroup\)/)
  assert.doesNotMatch(resourceSectionBody, /section\.groupNames\.map\(\(groupName\)/)
...
test('personal space workspace delegates storyboard management panel', () => {
  ...
  assert.match(panelSource, /expandedVoicePickerGroupId/)
