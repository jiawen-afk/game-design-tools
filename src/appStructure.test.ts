...
test('project management exposes remote database and qiniu kodo configuration', () => {
  ...
  assert.match(panelSource, /删除数据库配置/)
  assert.match(panelSource, /删除 Kodo 配置/)
...
test('project workspace routes remote migration and hard delete through project storage services', () => {
  ...
  assert.match(workspaceSource, /syncProjectSpaceStateToLocalProjectStorage/)
...
test('project workspace startup resolves migrated projects from the remote repository first', () => {
  ...
  assert.match(workspaceSource, /mergeProjectsRemoteFirst/)
  ...
  assert.match(workspaceSource, /const nextProjects = mergeProjectsRemoteFirst\(localProjects, remoteProjects\)/)
...
test('remote project workspace changes sync through project storage service', () => {
  ...
  assert.match(workspaceSource, /const syncActiveProjectStateToStorage = async \(nextSpace: PersonalSpaceState\)/)
  assert.match(workspaceSource, /project\?\.mode === 'remote'/)
  assert.match(workspaceSource, /repository: remoteProjectRepository/)
  assert.match(workspaceSource, /objectStorage: remoteProjectObjectStorage/)
  assert.match(workspaceSource, /assetManager: projectAssetManager/)
  assert.match(workspaceSource, /storageProvider: 'qiniu_kodo'/)
  assert.match(workspaceSource, /const remoteSettings = remoteSettingsForProject\(project\.id\)/)
  assert.match(workspaceSource, /databaseProvider: remoteSettings\.database_provider/)
  assert.match(workspaceSource, /remoteDatabaseProfileId: remoteSettings\.remote_database_profile_id/)
  assert.match(workspaceSource, /remoteStorageProfileId: remoteSettings\.remote_storage_profile_id/)
...
test('project workspace exposes manual sync status and blocks repeated sync clicks', () => {
  ...
  assert.match(workspaceSource, /const \[syncingProjectId, setSyncingProjectId\] = useState\(''\)/)
  assert.match(workspaceSource, /const manualSyncInFlightProjectIdRef = useRef\(''\)/)
  assert.match(workspaceSource, /const syncActiveProjectNow = async \(\)/)
  assert.match(workspaceSource, /if \(manualSyncInFlightProjectIdRef\.current\)/)
  assert.match(workspaceSource, /setSyncingProjectId\(syncProjectId\)/)
  assert.match(workspaceSource, /messageApi\.success\('已同步项目空间'\)/)
...
test('remote project realtime sync is debounced and has a periodic fallback', () => {
  ...
  assert.match(workspaceSource, /REMOTE_PROJECT_SYNC_DEBOUNCE_MS/)
  assert.match(workspaceSource, /REMOTE_PROJECT_PERIODIC_SYNC_MS/)
  assert.match(workspaceSource, /const remoteSyncTimerRef = useRef/)
  assert.match(workspaceSource, /const queuedRemoteSyncSpaceRef = useRef<PersonalSpaceState \| null>\(null\)/)
  assert.match(workspaceSource, /const remoteSyncInFlightRef = useRef\(false\)/)
  assert.match(workspaceSource, /const flushQueuedRemoteSync = async \(\)/)
  assert.match(workspaceSource, /const scheduleRemoteProjectSync = \(nextSpace: PersonalSpaceState, delay = REMOTE_PROJECT_SYNC_DEBOUNCE_MS\)/)
  assert.match(workspaceSource, /remoteSyncTimerRef\.current = window\.setTimeout/)
  assert.match(workspaceSource, /void flushQueuedRemoteSync\(\)/)
  assert.match(workspaceSource, /\}, delay\)/)
  assert.match(workspaceSource, /remoteSyncIntervalRef\.current = window\.setInterval/)
  assert.match(workspaceSource, /scheduleRemoteProjectSync\(spaceRef\.current, 0\)/)
  assert.match(workspaceSource, /\}, REMOTE_PROJECT_PERIODIC_SYNC_MS\)/)
  assert.match(workspaceSource, /scheduleRemoteProjectSync\(space\)/)
