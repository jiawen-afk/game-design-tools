...
test('app shell exposes project space instead of personal space copy', () => {
  ...
  assert.match(managementSource, /编辑项目/)
  assert.match(managementSource, /删除项目/)
  assert.match(managementSource, /本地模式/)
...
test('project migration exposes an in-progress state and blocks repeated clicks', () => {
  ...
  assert.match(panelSource, /loading=\{selectedProjectMigrating\}/)
  assert.match(panelSource, /disabled=\{[^}]*selectedProjectMigrating/)
  assert.match(panelSource, /selectedProjectMigrating \? '迁移中' : '迁移到远程'/)
