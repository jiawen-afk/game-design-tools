...
test('desktop API exposes typed video processing operations', () => {
  ...
  assert.match(apiSource, /getVideoOutputDirectory\(\): Promise<DesktopVideoDirectoryInfo \| null>/)
...
test('preload maps video operations to focused IPC channels', () => {
  ...
  assert.match(preloadSource, /getVideoOutputDirectory: \(\) => invoke\('video-processing:get-output-directory'\)/)
...
test('electron main delegates video processing IPC to a focused module', () => {
  ...
  assert.match(handlerSource, /video-processing:get-output-directory/)
  ...
  assert.match(handlerSource, /createVideoOutputDirectorySession/)
  assert.match(handlerSource, /const outputDirectorySession = createVideoOutputDirectorySession\(app\)/)
  assert.match(handlerSource, /video-processing:get-output-directory', async \(\) => outputDirectorySession\.restore\(\)/)
  assert.match(handlerSource, /const rememberedDirectory = await outputDirectorySession\.restore\(\)/)
  assert.match(handlerSource, /defaultPath: rememberedDirectory\?\.path/)
  assert.match(handlerSource, /outputDirectorySession\.remember\(selectedPath\)/)
  assert.match(handlerSource, /await outputDirectorySession\.restore\(\)/)
  assert.match(handlerSource, /outputDirectorySession\.selectedOutputDirectories/)
  assert.doesNotMatch(handlerSource, /const selectedOutputDirectories = new Set\(\)/)
