...
test('home page exposes renamed workspaces and global project space', () => {
  const source = [
    ...
    readFileSync('src/components/VideoProcessingWorkspace/index.tsx', 'utf8'),
  ].join('\n')
...
test('home page exposes image processing workspace as a first-class tool', () => {
  ...
  assert.match(navigationSource, /export type ToolId = 'multi-frame-sprite' \| 'image-processing' \| 'voice-deployment' \| 'video-processing'/)
  ...
  assert.match(navigationSource, /export const personalSpaceShortcut = '5'/)
})
