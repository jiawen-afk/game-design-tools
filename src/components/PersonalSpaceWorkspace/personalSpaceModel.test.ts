...
test('creates common resource assets from uploaded files', () => {
  const map = createResourceAssetFromUpload({
    kind: 'map',
    name: 'forest.png',
  ...
  assert.equal(map.groupName, '地图素材')
  assert.deepEqual(map.tags, ['地图', '森林'])
  assert.equal(effect.groupName, '特效素材')
  assert.deepEqual(effect.tags, ['特效'])
  assert.equal(voice.groupName, '配音素材')
  assert.deepEqual(voice.tags, ['配音'])
})

test('personal space assets default to a named group', () => {
  const asset = createPersonalSpaceAsset({ kind: 'effect', name: '火焰爆炸' })
...
test('effect assets can link voice assets, and storyboard voice entries export dialogue order', () => {
  const effect = createPersonalSpaceAsset({ kind: 'effect', name: '火球' })
  ...
  state = linkEffectAssetToVoice(state, effect.id, voice.id)
...
test('assets archived with a storage directory receive categorized target paths', () => {
  ...
  assert.deepEqual(archived.storageResourcePaths, [
    'D:\\GameAssets\\角色精灵图\\主角行走\\sprite.png',
    'D:\\GameAssets\\角色精灵图\\主角行走\\index.json',
  ])
})
...
test('portrait assets archived with a storage directory use the character portrait category', () => {
  ...
  assert.deepEqual(archived.storageResourcePaths, [
    'D:\\GameAssets\\角色肖像\\主角头像\\portrait.png',
  ])
})
