...
test('imports entity graph files as normalized document rows without raw JSON source storage', async () => {
  ...
  const capturedInput = replacedInput as unknown as ReplaceDocumentGraphInput
  assert.equal(capturedInput.sourceContents.length, 1)
  assert.equal(capturedInput.sourceContents[0]?.source_id, capturedInput.sources[0]?.id)
  assert.equal(capturedInput.sourceContents[0]?.content_text, fileText)
  assert.equal(capturedInput.sourceContents[0]?.content_encoding, 'utf-8')
  assert.equal(capturedInput.sourceContents[0]?.size_bytes, file.size)
  assert.equal(capturedInput.sourceContents[0]?.hash_sha256, capturedInput.sources[0]?.hash_sha256)
  assert.equal(capturedInput.sources[0] && 'content_text' in capturedInput.sources[0], false)
