...
test('legacy migration converts assets, groups, character links, storyboard entries, and asset relations', () => {
  ...
  assert.equal(migratedVoice.primary_object_key.startsWith('objects/p1/audio/'), true)
  assert.equal(migratedEffect.primary_object_key.startsWith('objects/p1/image/'), true)
