...
test('sprite playback panel exposes batch upscale controls and a side by side upscale preview', () => {
  ...
  assert.match(upscalePanel, /GPU 设备/)
  assert.match(upscalePanel, /线程配置/)
  assert.match(upscalePanel, /updateUpscaleOptions\(\{\s*gpuId/)
  assert.match(upscalePanel, /updateUpscaleOptions\(\{\s*threadProfile/)
