...
test('layout workspace panel delegates canvas editing view details', () => {
  ...
  assert.match(activeFrameLayer, /onDragStart=/)
  assert.match(activeFrameLayer, /preventDefault\(\)/)
  assert.match(activeFrameLayer, /touchAction:\s*'none'/)
  assert.match(activeFrameLayer, /WebkitUserDrag:\s*'none'/)
