...
- **视频处理工作台**：按百分比或目标分辨率缩放，使用 Upscayl GPU 超分，按质量或目标大小压缩，并导出 OGV、WebM 或 MP4。
...
- 视频工作台使用 **FFmpeg 和 FFprobe** 完成媒体探测、解码、压缩与 OGV/WebM/MP4 输出
...
- **视频处理工作台**：串行批量缩放、GPU 超分、按格式压缩、可选音频、帧率控制、导出目录记忆、Godot 4.6 原生 OGV 导出，以及 WebM/MP4 通用交付格式。
...
视频工作台默认并推荐为 Godot 导出 Ogg 容器、Theora `yuv420p` 视频和可选 Vorbis 音频；同时可导出 VP9/Opus WebM 与 OpenH264/AAC MP4，供浏览器、常规播放器和素材交换使用。Godot 4.6 原生 `VideoStreamPlayer` 仍应选择 OGV；WebM 与 MP4 需要另行接入播放方案。

1. 使用快捷键 `4` 打开 **视频处理工作台**。
2. 安装固定版本的 FFmpeg/FFprobe 运行包。
3. 启用 GPU 超分前安装 **Upscayl GPU**。
4. 导入一个或多个视频并选择导出目录；视频工作台会记住最近授权的目录。
5. 选择 OGV、WebM 或 MP4，再设置缩放百分比/分辨率、压缩模式、目标帧率、音频模式和可选 Upscayl 高级参数。
6. 启动队列。任务串行执行；单项失败不会阻止后续任务。
7. 如需 Godot 原生播放，把生成的 `.ogv` 加入 Godot 4.6 项目，并赋值给 `VideoStreamPlayer.stream`。

在相近画质下，Theora 的压缩效率低于 VP9 或 H.264，但它仍是 Godot 原生播放选项。每种格式发布前都会校验对应容器、视频/音频编码、分辨率、帧率、媒体流数量，以及是否存在过长的音频或容器尾部。
...
视频运行包使用固定的 LGPL shared FFmpeg 构建，并包含工作台使用的 libvpx、Opus 与 OpenH264 等编码实现。软件许可证不能代替编码器专利判断：如果产品分发或商业使用 H.264/AAC、H.265/HEVC 等可能受专利约束的编码器，使用者仍需根据自身产品和所在地区评估授权义务。本工具不输出 H.265。
...
- `src/components/VideoProcessingWorkspace`：视频缩放、GPU 超分、压缩、串行队列、导出目录记忆、验证和 OGV/WebM/MP4 导出
