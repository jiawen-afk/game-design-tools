...
- **精灵图工作台**：导入多张图片、整张精灵图或视频片段，完成去背、统一画布、逐帧对齐、播放预览、排序和导出。流程 4 高清化互斥分为输入图高清化和结果图高清化：输入图高清化用于修复模糊源图，会先高清化抠图源图再重新应用流程 3 的画布、布局、描边和轮廓参数；结果图高清化会放大流程 3 合成后的结果帧，并按放大后的单帧尺寸导出。
...
- **精灵图工作台**：去背、帧对齐、精灵图预览、帧排序、高清化前后并排播放，互斥的输入图/结果图批量高清化，以及原始或高清化导出。
